...
class Translator:
    ...
    def _build_prompt(self, input_data, json_module, from_lang="en"):
...
You MUST output ONLY a valid JSON array of objects, containing two keys: 'index' (int) and 'text' (string, the translated Vietnamese text). Do not wrap the JSON in code blocks (such as ```json) or add any extra text or conversational filler."""
    ...
    def _call_gemini(self, input_data, json_module, urllib_request, GEMINI_API_KEY, from_lang="en"):
        """Gọi Gemini API cho một batch nhỏ."""
        prompt = self._build_prompt(input_data, json_module, from_lang)
    ...
    def _call_github(self, input_data, json_module, urllib_request, GITHUB_TOKEN, from_lang="en"):
    ...
    def _call_sambanova(self, input_data, json_module, urllib_request, SAMBANOVA_API_KEY, from_lang="en"):
    ...
    def _call_groq(self, input_data, json_module, urllib_request, GROQ_API_KEY, from_lang="en"):
    ...
    def translate_segments(self, segments: list, from_lang: str = "en", progress_callback=None) -> list:
        """
        ...
        if GEMINI_API_KEY and from_lang != "vi":
            api_providers.append(("Gemini", lambda batch, fl=from_lang: self._call_gemini(batch, json, urllib.request, GEMINI_API_KEY, fl)))
        if GITHUB_TOKEN and from_lang != "vi":
            api_providers.append(("GitHub GPT-4o-mini", lambda batch, fl=from_lang: self._call_github(batch, json, urllib.request, GITHUB_TOKEN, fl)))
        if SAMBANOVA_API_KEY and from_lang != "vi":
            api_providers.append(("SambaNova Llama 3.1", lambda batch, fl=from_lang: self._call_sambanova(batch, json, urllib.request, SAMBANOVA_API_KEY, fl)))
        if GROQ_API_KEY and from_lang != "vi":
            api_providers.append(("Groq Llama 3.3", lambda batch, fl=from_lang: self._call_groq(batch, json, urllib.request, GROQ_API_KEY, fl)))
